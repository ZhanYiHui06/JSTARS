...
from .conv import Conv, DSConv, DWConv, GhostConv, LightConv, autopad
...
class RepC3(nn.Module):
    ...
    def __init__(self, c1, c2, n=3, e=1.0):
        ...
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c1, c_, 1, 1)
        self.m = nn.Sequential(*[SRConv(c_, c_) for _ in range(n)])
...
class DSBottleneck(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, e=0.5, k1=3, k2=5, d2=1):
        """Initializes DSBottleneck with depthwise separable convolutions and optional shortcut."""
        super().__init__()
        c_ = int(c2 * e)
    ...
    def forward(self, x):
        """Forward pass through the DSBottleneck module."""
...
class LSKBlock(nn.Module):
    """
    Large Selective Kernel Block for remote sensing object detection.
    
    From LSKNet: A Foundation Lightweight Backbone for Remote Sensing (IJCV 2024, ICCV 2023)
    GitHub: https://github.com/zcablii/LSKNet
    
    This module dynamically adjusts large spatial receptive field to better model
    the ranging context of various objects in remote sensing scenarios.
    
    Args:
        c1 (int): Number of input channels.
        c2 (int): Number of output channels (default: same as c1).
    """
    ...
    def forward(self, x):
        """Forward pass through LSK block."""
        attn1 = self.conv0(x)
...
class SGE(nn.Module):
    """
    Smoothed Gradient Enhancement (SGE) module for SAR image detection.
    
    This module follows the classical "smooth denoising - edge enhancement" approach
    from traditional image processing, integrated with modern neural network design.
    
    Components:
        1. GConv: Gaussian convolution for noise suppression (fixed kernel)
        2. SConv: Scharr convolution for edge extraction (fixed kernel)
        3. Edge Integration: Gradient magnitude computation
        4. Conv Block: Learnable feature enhancement
    
    Args:
        c1 (int): Number of input channels.
        c2 (int): Number of output channels (default: same as c1).
        kernel_size (int): Size of Gaussian kernel (default: 3).
        sigma (float): Standard deviation for Gaussian kernel (default: 1.0).
    """
    
    def __init__(self, c1, c2=None, kernel_size=3, sigma=1.0):
        super().__init__()
        c2 = c2 or c1
        self.c1 = c1

        gaussian_kernel = self._create_gaussian_kernel(kernel_size, sigma)
        gaussian_kernel = gaussian_kernel.repeat(c1, 1, 1, 1)
        ...
        scharr_x = scharr_x.view(1, 1, 3, 3).repeat(c1, 1, 1, 1)
        scharr_y = scharr_y.view(1, 1, 3, 3).repeat(c1, 1, 1, 1)
    ...
    def forward(self, x):
        """Forward pass through SGE module."""
        f_gaussian = F.conv2d(x, self.gaussian_kernel, padding=self.gaussian_padding, groups=self.c1)
        ix = F.conv2d(f_gaussian, self.scharr_x, padding=1, groups=self.c1)
        iy = F.conv2d(f_gaussian, self.scharr_y, padding=1, groups=self.c1)
        f_edge = torch.sqrt(ix**2 + iy**2 + 1e-6)
        f_add = x + f_edge
        f_out = self.conv_block(f_add)
        return f_out + self.shortcut(f_add)
...
class SELayer(nn.Module):
    ...
    def __init__(self, channel, reduction=16):
        """Initializes the SE layer with adaptive average pooling and fully connected layers."""
        super().__init__()
    ...
    def forward(self, x):
        """Forward pass through the SE layer with channel-wise recalibration."""
        b, c, _, _ = x.size()
...
class SFM(nn.Module):
    """
    Spectral Feature Modulation (SFM) module.
    
    Processes frequency-domain features with adaptive filtering on both
    amplitude and phase components.
    
    Args:
        c1 (int): Number of input channels.
        reduction (int): Reduction ratio for SE layer (default: 16).
    """
    
    def __init__(self, c1, reduction=16):
        super().__init__()
        self.amp_conv = nn.Conv2d(c1, c1, 1)
        self.phase_conv = nn.Conv2d(c1, c1, 1)
        self.se_amp = SELayer(c1, reduction=max(reduction, c1 // 4))
        self.se_phase = SELayer(c1, reduction=max(reduction, c1 // 4))

    def forward(self, x):
        """
        Forward pass through SFM module.

        Args:
            x: Complex tensor from FFT (B, C, H, W//2+1)

        Returns:
            Complex tensor with enhanced frequency features
        """
        amp = x.abs()
        phase = x.angle()
        amp_filtered = torch.sigmoid(self.amp_conv.float()(amp))
        phase_filtered = torch.sigmoid(self.phase_conv.float()(phase))
        amp_out = self.se_amp.float()(amp_filtered) + amp
        phase_out = self.se_phase.float()(phase_filtered) + phase
...
class SRConv(nn.Module):
    """
    Structural Re-parameterization Convolution (SRConv).

    During training, this module uses three parallel branches (3x3, 1x1, and identity) that are
    fused into a single 3x3 convolution during inference for efficient deployment.

    Attributes:
        c1 (int): Number of input channels.
        c2 (int): Number of output channels.
        stride (int, optional): Convolution stride. Defaults to 1.
        deploy (bool, optional): If True, use the fused single-convolution inference mode. Defaults to False.

    Methods:
        forward: Forward pass through the SRConv module.
        switch_to_deploy: Fuses training branches into a single convolution for inference.

    Examples:
        >>> import torch
        >>> model = SRConv(c1=64, c2=64)
        >>> x = torch.randn(2, 64, 32, 32)
        >>> output = model(x)
        >>> print(output.shape)
        torch.Size([2, 64, 32, 32])
    ...
    def __init__(self, c1, c2, stride=1, deploy=False):
        """Initializes SRConv with optional deploy mode for fused inference."""
        super().__init__()
        self.deploy = deploy
        self.c1 = c1
        self.c2 = c2
        ...
        if deploy:
            self.rbr_reparam = nn.Conv2d(c1, c2, 3, stride, 1, bias=True)
        else:
    ...
    def forward(self, inputs):
        """Forward pass through SRConv layer."""
        if self.deploy:
    ...
    def switch_to_deploy(self):
        """Fuses the training branches into a single 3x3 convolution for efficient inference."""
        if self.deploy:
            return
        kernel, bias = self.get_equivalent_kernel_bias()
        self.rbr_reparam = nn.Conv2d(self.c1, self.c2, 3, self.stride, 1, bias=True)
    ...
    def get_equivalent_kernel_bias(self):
        """Computes the equivalent kernel and bias by fusing all branches."""
        ...
        return (
            k3x3 + self._pad_1x1_to_3x3_tensor(k1x1) + kid,
            b3x3 + b1x1 + bid
        )

    def _pad_1x1_to_3x3_tensor(self, k1x1):
        """Pads a 1x1 kernel tensor to 3x3 size."""
        if k1x1 == 0: return 0
        return torch.nn.functional.pad(k1x1, [1, 1, 1, 1])

    def _fuse_bn_tensor(self, branch):
        """Fuses a convolution and batch normalization into a single kernel and bias."""
        ...
        if isinstance(branch, nn.Sequential):
            ...
            if not hasattr(self, 'id_tensor'):
                input_dim = self.c1 // self.groups if hasattr(self, 'groups') else self.c1
...
class LRE(nn.Module):
    """
    Local Reparameterization Encoder (LRE).

    A lightweight encoder composed of stacked SRConv blocks with optional channel projection.

    Attributes:
        c1 (int): Number of input channels.
        c2 (int): Number of output channels.
        n (int, optional): Number of stacked SRConv blocks. Defaults to 3.

    Methods:
        forward: Forward pass through the LRE module.

    Examples:
        >>> import torch
        >>> model = LRE(c1=64, c2=64, n=3)
        >>> x = torch.randn(2, 64, 32, 32)
        >>> output = model(x)
        >>> print(output.shape)
        torch.Size([2, 64, 32, 32])
    ...
    def __init__(self, c1, c2, n=3):
        """Initializes the LRE with optional channel projection and stacked SRConv blocks."""
        super().__init__()
        self.conv_in = nn.Conv2d(c1, c2, 1) if c1 != c2 else nn.Identity()
        self.blocks = nn.Sequential(*[
            SRConv(c2, c2) for _ in range(n)
        ])
        
    def forward(self, x):
        """Forward pass through LRE module."""
        return self.blocks(self.conv_in(x))


class D3Block(nn.Module):
    """
    Dual-Domain Denoising Block (D3) for SAR images.
    
    This module processes features through two parallel branches:
        1. Spatial Branch: LRE for spatial-domain feature enhancement
        2. Frequency Branch: FFT -> SFM -> IFFT for frequency-domain denoising
    
    The outputs are concatenated and fused for complementary information utilization.
    
    Args:
        c1 (int): Number of input channels (usually from concatenated multi-scale features).
        c2 (int): Number of output channels.
    """
    ...
    def __init__(self, c1, c2=None):
        ...
        self.spatial_conv = nn.Conv2d(c1, c_, 1)
        self.lre = LRE(c_, c_, n=1)

        self.freq_conv = nn.Conv2d(c1, c_, 1)
        self.sfm = SFM(c_)

        self.out_conv = nn.Conv2d(c_ * 2, c2, 1)
        self.bn = nn.BatchNorm2d(c2)
        self.act = nn.SiLU()

    def forward(self, x):
        """Forward pass through D3Block module."""
        spatial = self.lre(self.spatial_conv(x))

        freq_in = self.freq_conv(x)
        orig_dtype = freq_in.dtype
        freq_in = freq_in.float()
        freq = torch.fft.rfft2(freq_in, norm='ortho')
        freq_out = self.sfm(freq)
        freq_spatial = torch.fft.irfft2(freq_out, s=freq_in.shape[-2:], norm='ortho')
        freq_spatial = freq_spatial.to(orig_dtype)

        out = torch.cat([spatial, freq_spatial], dim=1)
        out = self.act(self.bn(self.out_conv(out)))
        return out
